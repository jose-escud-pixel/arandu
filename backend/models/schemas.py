...
class EmpresaCreate(BaseModel):
    ...
    logo_tipo: str = "arandujar"         # arandu | jar | arandujar — empresa propia a la que pertenece
...
class EmpresaPropiaCreate(BaseModel):
    ...
    tema: Optional[str] = "oscuro-azul"   # oscuro-azul | oscuro-rojo | claro-rojo | claro-dorado | claro-azul
...
class ConceptoItem(BaseModel):
    """Un ítem/concepto dentro de una factura con múltiples conceptos."""
    descripcion: str
    monto: float
...
class FacturaCreate(BaseModel):
    ...
    ruc: Optional[str] = None
    concepto: str
    conceptos: List[ConceptoItem] = []    # lista de conceptos cuando no hay vínculo presupuesto/contrato
    monto: float
    moneda: str = "PYG"
    tipo_cambio: Optional[float] = None
