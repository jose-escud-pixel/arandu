...
async def get_empresas_propias(user: dict = Depends(require_authenticated)):
    ...
    if not propias:
        defaults = [
            {"id": str(uuid.uuid4()), "nombre": "Arandu",    "slug": "arandu",    "color": "#3B82F6", "logo_url": None, "created_at": datetime.now(timezone.utc).isoformat()},
            {"id": str(uuid.uuid4()), "nombre": "JAR",       "slug": "jar",       "color": "#10B981", "logo_url": None, "created_at": datetime.now(timezone.utc).isoformat()},
            {"id": str(uuid.uuid4()), "nombre": "AranduJAR", "slug": "arandujar", "color": "#8B5CF6", "logo_url": None, "created_at": datetime.now(timezone.utc).isoformat()},
        ]
