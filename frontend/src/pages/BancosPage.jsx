...
export default function BancosPage() {
  ...
  const [cuentas, setCuentas] = useState([]);   // con saldo_actual
  ...
  useEffect(() => { fetchCuentas(); }, [activeEmpresaPropia]); // eslint-disable-line
  ...
  const totalPYG = useMemo(() => cuentas.filter(c => c.moneda === "PYG").reduce((s, c) => s + (c.saldo_actual || 0), 0), [cuentas]);
  const totalUSD = useMemo(() => cuentas.filter(c => c.moneda === "USD").reduce((s, c) => s + (c.saldo_actual || 0), 0), [cuentas]);
            ...
            {cuentas.map(c => (
              <div key={c.id} data-testid={`cuenta-${c.id}`}
                ...
                <div className="text-right">
                  <p className={`font-heading text-lg font-bold ${c.saldo_actual >= 0 ? "text-emerald-300" : "text-red-300"}`}>
