...
const MODULOS_LABELS = {
  ...
  inventario_productos: "Productos (Inventario)",
  compras: "Compras / Egresos",
...
const UsuariosPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        empresa_default: formData.empresa_default || null
      };
  ...
  const handleEdit = (usuario) => {
    ...
    setFormData({
      ...
      empresa_default: usuario.empresa_default || ""
    });
    setShowForm(true);
  };
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {formData.role === "usuario" && (
                    <div className="space-y-4 pt-2">
                      ...
                      {/* Nota informativa: acceso por empresa */}
                      <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-3 text-xs text-blue-300">
                        <span className="font-semibold">ℹ️ Acceso a clientes:</span> el usuario verá automáticamente todos los clientes de las empresas asignadas arriba. No es necesario seleccionar clientes individuales.
                      </div>
                  ...
                  {(() => {
                    // Para admins: puede elegir cualquier empresa o ninguna (ver todas)
                    // Para usuarios: solo sus empresas asignadas (logos_asignados)
                    const opcionesEmpresa = formData.role === "admin"
                      ? empresasPropias
                      : empresasPropias.filter(ep => (formData.logos_asignados || []).includes(ep.id));
                    if (opcionesEmpresa.length === 0) return null;
                    return (
                      <div>
                        <label className="block text-slate-400 text-sm mb-2 font-body">
                          Empresa predeterminada al iniciar sesión
                        </label>
                        <select
                          value={formData.empresa_default || ""}
                          onChange={e => setFormData({ ...formData, empresa_default: e.target.value || null })}
                          className="w-full bg-arandu-dark border border-white/10 rounded-lg px-3 py-2 text-white text-sm font-body focus:outline-none focus:border-arandu-blue"
                        >
                          {formData.role === "admin" && (
                            <option value="">Sin preferencia (ver todas las empresas)</option>
                          )}
                          {formData.role !== "admin" && (
                            <option value="">Sin preferencia (primera empresa asignada)</option>
                          )}
                          {opcionesEmpresa.map(ep => (
                            <option key={ep.id} value={ep.id}>{ep.nombre}</option>
                          ))}
                        </select>
                        <p className="text-slate-500 text-xs mt-1">
                          Esta empresa se activará automáticamente cuando el usuario inicia sesión.
                        </p>
                      </div>
                    );
                  })()}
