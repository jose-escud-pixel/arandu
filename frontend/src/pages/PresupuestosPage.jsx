...
import {
  ...
  XCircle, Send, Copy, Edit, Receipt, DollarSign,
...
import { svgMarcaIcon, svgTriClipText, normalizeLogoTipo } from "../lib/marcaLogoSvg";
import { toast } from "sonner";

const API = `${process.env.REACT_APP_BACKEND_URL}/api`;

// Logo Components (letras tricolor PY; icono cuadrado por marca)
const LogoArandu = () => <LogoMarcaArandu />;
const LogoJar = () => <LogoMarcaJar />;
const LogoAranduJar = () => <LogoMarcaAranduJar />;
...
const PresupuestosPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [presRes, empRes, provRes, propiasRes, prodRes] = await Promise.all([
        ...
        fetch(`${API}/admin/empresas${logoTipoActivo ? `?logo_tipo=${logoTipoActivo}` : ""}`, {
          headers: { Authorization: `Bearer ${token}` }
        }),
  ...
  // Logo para impresión: texto coloreado simple (sin SVG clipPath que falla al imprimir/PDF)
  const buildLogoSVG = (logoTipo) => {
    const uid = Math.random().toString(36).slice(2, 11);
    const marca = normalizeLogoTipo(logoTipo);
    const iconSVG = svgMarcaIcon(marca, uid);
    const baseStyle = `font-family:Arial,Helvetica,sans-serif;font-weight:900;line-height:1;`;
    let nombreHTML = "";
    if (marca === "jar") {
      nombreHTML = `<span style="${baseStyle}font-size:28px;letter-spacing:2px;color:#f87171">JAR</span>`;
    } else if (marca === "arandujar") {
      nombreHTML = `<span style="${baseStyle}font-size:18px;letter-spacing:0.5px;"><span style="color:#93c5fd">ARANDU</span><span style="color:#f87171">&amp;JAR</span></span>`;
    } else {
      nombreHTML = `<span style="${baseStyle}font-size:26px;letter-spacing:2px;color:#93c5fd">ARANDU</span>`;
    }
    return `<div style="display:flex;align-items:center;gap:8px">
      ${iconSVG}
      <div style="line-height:1">
        <div style="margin:0;padding:0">${nombreHTML}</div>
        <div style="font-size:9px;color:#94a3b8;letter-spacing:3px;text-transform:uppercase;margin-top:2px">INFORMÁTICA</div>
      </div>
    </div>`;
  };
  ...
  */
  const buildLogoPngDataUrl = (logoTipo, brandName) => {
    try {
      const marca = normalizeLogoTipo(logoTipo);
      // Configuración por marca conocida; fallback para slugs nuevos
      const configs = {
        jar: { text: "JAR", fontSize: 66, iconGrad: ["#ef4444", "#7f1d1d"], iconShape: "server" },
        arandu: { text: "ARANDU", fontSize: 58, iconGrad: ["#3b82f6", "#1e3a8a"], iconShape: "cpu" },
        arandujar: { text: "ARANDU&JAR", fontSize: 40, iconGrad: ["#3b82f6", "#1e3a8a"], iconShape: "server" },
      };
      const fallbackText = (brandName || logoTipo || "LOGO").toUpperCase().slice(0, 16);
      const cfg = configs[marca] || {
        text: fallbackText,
        fontSize: fallbackText.length > 8 ? 40 : 56,
        iconGrad: ["#3b82f6", "#1e3a8a"],
        iconShape: "server",
      };
      ...
      const iconSz = 88;
      const gap = 16;
      // Medir texto para dimensionar el canvas correctamente
      const measure = document.createElement("canvas").getContext("2d");
      measure.font = `900 ${cfg.fontSize}px Arial, Helvetica, sans-serif`;
      const textW = Math.ceil(measure.measureText(cfg.text).width) + 8;
      const subW = 130;
      const contentW = iconSz + gap + Math.max(textW, subW);
      const contentH = 96;
      ...
      grad.addColorStop(0, cfg.iconGrad[0]);
      grad.addColorStop(1, cfg.iconGrad[1]);
      ctx.fillStyle = grad;
      ctx.beginPath();
      ctx.moveTo(iconX + radius, iconY);
      ctx.lineTo(iconX + iconSz - radius, iconY);
      ctx.quadraticCurveTo(iconX + iconSz, iconY, iconX + iconSz, iconY + radius);
      ctx.lineTo(iconX + iconSz, iconY + iconSz - radius);
      ctx.quadraticCurveTo(iconX + iconSz, iconY + iconSz, iconX + iconSz - radius, iconY + iconSz);
      ctx.lineTo(iconX + radius, iconY + iconSz);
      ctx.quadraticCurveTo(iconX, iconY + iconSz, iconX, iconY + iconSz - radius);
      ctx.lineTo(iconX, iconY + radius);
      ctx.quadraticCurveTo(iconX, iconY, iconX + radius, iconY);
      ctx.closePath();
      ctx.fill();

      // Detalle interno del ícono (server/cpu simplificado, en blanco)
      ctx.strokeStyle = "#ffffff";
      ctx.lineWidth = 2.2;
      if (cfg.iconShape === "cpu") {
        const cx = iconX + iconSz / 2, cy = iconY + iconSz / 2;
        ...
        ctx.beginPath();
        [-10, 0, 10].forEach(dx => {
          ctx.moveTo(cx + dx, cy - 16); ctx.lineTo(cx + dx, cy - 22);
          ctx.moveTo(cx + dx, cy + 16); ctx.lineTo(cx + dx, cy + 22);
          ctx.moveTo(cx - 16, cy + dx); ctx.lineTo(cx - 22, cy + dx);
          ctx.moveTo(cx + 16, cy + dx); ctx.lineTo(cx + 22, cy + dx);
        });
        ctx.stroke();
      } else {
        // server: dos bandejas con LEDs
        const cx = iconX + iconSz / 2, cy = iconY + iconSz / 2;
        ctx.strokeRect(cx - 18, cy - 16, 36, 12);
        ctx.strokeRect(cx - 18, cy + 2, 36, 14);
        ctx.fillStyle = "#ffffff";
        ctx.beginPath();
        ctx.arc(cx - 12, cy - 10, 1.8, 0, Math.PI * 2);
        ctx.arc(cx - 6, cy - 10, 1.8, 0, Math.PI * 2);
        ctx.arc(cx - 12, cy + 9, 1.8, 0, Math.PI * 2);
        ctx.arc(cx - 6, cy + 9, 1.8, 0, Math.PI * 2);
        ctx.fill();
      }
      ...
      let sx = textX;
      const sy = textTop + textH + 8;
      for (const ch of sub) {
        ctx.fillText(ch, sx, sy);
        sx += ctx.measureText(ch).width + 2.2;
      }

      return canvas.toDataURL("image/png");
    } catch (e) {
      console.error("Error generando logo PNG", e);
      return null;
    }
  };

  // Encabezado "limpio" para la impresión completa:
  //  - logo como PNG (rasterizado, se imprime igual en cualquier navegador/impresora)
  //  - SIN textos pequeños de dirección / teléfono / email
  //  - color de acento por empresa (JAR=rojo, demás=azul)
  const buildHeaderHTMLClean = (logoTipo, numero, fecha, validezDias, brandName) => {
    ...
    const logoPng = buildLogoPngDataUrl(logoTipo, brandName);
    const logoImg = logoPng
      ? `<img src="${logoPng}" alt="logo" style="display:block;height:72px;width:auto;max-width:340px"/>`
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;position:relative;z-index:1;gap:20px">
        ...
        <div style="text-align:right">
          <div style="background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.18);border-radius:10px;padding:12px 20px">
            ...
            <div style="font-size:22px;font-weight:900;color:#ffffff;letter-spacing:1px">${numero}</div>
            ...
            <div style="font-size:11px;color:#cbd5e1">Fecha: ${fecha}</div>
            <div style="font-size:11px;color:#cbd5e1">Válido ${validezDias} días</div>
          </div>
        </div>
      </div>
    </div>
  ...
  const handlePrintCompleto = () => {
    ...
    const brandName = (empresasPropias.find(e => e.slug === showPreview.logo_tipo)?.nombre) || showPreview.logo_tipo || "";
...
<body>
  ${buildHeaderHTMLClean(showPreview.logo_tipo, showPreview.numero, showPreview.fecha, showPreview.validez_dias, brandName)}

  <div class="content">
  <!-- Cliente (bloque protegido contra corte) -->
  <div class="no-break" style="background:${accentBg};padding:10px 14px;margin-bottom:14px;border-left:4px solid ${accentColor};border-radius:0 6px 6px 0">
  ...
  <!-- Tabla de ítems -->
  <table style="margin-bottom:8px;border-radius:8px;overflow:hidden;border:1px solid #e2e8f0">
    <thead>
    ...
    <tfoot>
      <tr class="no-break" style="background:${accentBg}">
      ...
      <tr class="no-break" style="background:${accentColor}">
...
<head>
  ...
  <style>
    @page { size: A4; margin: 0; }
    * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    body { font-family: 'Segoe UI', Arial, Helvetica, sans-serif; margin: 0; padding: 0; background: white; color: #1e293b; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
              ...
              <table className="w-full">
                <thead>
                  <tr className="text-slate-400 text-xs border-b border-white/10">
                    ...
                    <th className="text-right p-2 w-32">
                      Diferencia
                      <span className="block text-slate-400 font-normal">{showCostos.moneda === "PYG" ? "₲" : "USD"}</span>
                    </th>
