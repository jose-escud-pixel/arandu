...
import {
  ArrowLeft, Download, FileText, Lock, Building2, Plus, X, Save, Edit, Trash2,
...
<body>
  ...
  const fetchAll = async () => {
    try {
      const [empRes, alertRes, proxRes, actRes] = await Promise.all([
        ...
        fetch(`${API}/admin/alertas${empresaFromUrl ? `?empresa_id=${empresaFromUrl}` : ""}`, { headers: { Authorization: `Bearer ${token}` } }),
        fetch(`${API}/admin/alertas/proximas`, { headers: { Authorization: `Bearer ${token}` } }),
  ...
  const openNewAlert = () => {
    ...
    setAlertForm({ empresa_id: filterEmpresa || empresaFromUrl || "", tipo: "dominio", nombre: "", descripcion: "", fecha_vencimiento: "", activo_id: "", notificar_dias: 30 });
  ...
  const filteredAlertas = useMemo(() => alertas.filter(a => !filterEmpresa || a.empresa_id === filterEmpresa), [alertas, filterEmpresa]);
    ...
    <div className="p-4 md:p-8">
        ...
        <div id="report-modal-overlay" className="fixed inset-0 bg-black/90 z-50 overflow-y-auto">
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", borderBottom: "2px solid #1e3a5f", paddingBottom: "10px", marginBottom: "14px" }}>
              <div>
                <span style={{ fontSize: "15px", fontWeight: "900", color: "#1e3a5f", letterSpacing: "-0.5px" }}>ARANDU</span>
                <span style={{ fontSize: "15px", fontWeight: "900", color: "#ef4444" }}>&</span>
                <span style={{ fontSize: "15px", fontWeight: "900", color: "#1e3a5f" }}>JAR </span>
                <span style={{ fontSize: "10px", color: "#6b7280", fontWeight: "normal" }}>INFORMATICA</span>
              </div>
              ...
              <div style={{ textAlign: "right", fontSize: "9px", color: "#555", minWidth: "200px" }}>
                ...
                {selectedEmpresasNombres && <div style={{ marginTop: "2px" }}>Filtros — <b>Empresa:</b> {selectedEmpresasNombres}</div>}
      ...
      {alertasProximas.length > 0 && (
        <div className="mb-6 bg-red-500/5 border border-red-500/20 rounded-xl p-4">
          <h3 className="text-red-400 font-semibold text-sm flex items-center gap-2 mb-3"><AlertTriangle className="w-4 h-4" /> Alertas Proximas ({alertasProximas.length})</h3>
      ...
      <div className="flex gap-2 mb-6">
        ...
        <button onClick={() => setTab("alertas")} className={`px-4 py-2 rounded-lg font-medium text-sm flex items-center gap-2 transition-all ${tab === "alertas" ? "bg-red-500/20 text-red-400 border border-red-500/30" : "bg-arandu-dark-lighter text-slate-400 border border-white/5"}`} data-testid="tab-alertas">
          <Bell className="w-4 h-4" /> Alertas {alertasProximas.length > 0 && <span className="bg-red-500 text-white text-xs w-5 h-5 rounded-full flex items-center justify-center">{alertasProximas.length}</span>}
        </button>
      </div>
        ...
        <div className="space-y-6">
            ...
            {[
              {
                ...
                icon: BarChart3,
                color: "emerald",
                desc: "Balance, facturas, presupuestos e ingresos",
                links: [
                  { label: "Balance mensual", href: "/admin/balance", icon: TrendingUp },
                  { label: "Facturas", href: "/admin/ventas", icon: Receipt },
                  { label: "Presupuestos", href: "/admin/ventas", icon: ClipboardList },
                  { label: "Egresos / Compras", href: "/admin/egresos", icon: TrendingDown },
                ],
              },
              {
                id: "inventario",
                ...
                icon: Package,
                color: "blue",
                desc: "Stock de productos y movimientos",
                links: [
                  { label: "Productos y stock", href: "/admin/inventario", icon: Package },
                  { label: "Compras con stock", href: "/admin/egresos", icon: DollarSign },
                ],
              },
              {
                id: "tecnico",
                ...
                icon: Server,
                color: "amber",
                desc: "Inventario técnico, activos y credenciales",
                links: null, // usa el panel de abajo
              },
            ].map(cat => {
              const Icon = cat.icon;
              ...
              return (
                <button key={cat.id} onClick={() => setReporteCategoria(cat.id)}
                  className={`text-left p-4 rounded-xl border transition-all ${active
                    ? `bg-${cat.color}-500/15 border-${cat.color}-500/40`
                    : "bg-arandu-dark-light border-white/5 hover:border-white/20"}`}
                >
                  <div className="flex items-center gap-3 mb-2">
                    <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${active ? `bg-${cat.color}-500/20` : "bg-white/5"}`}>
                      <Icon className={`w-5 h-5 ${active ? `text-${cat.color}-400` : "text-slate-400"}`} />
                    </div>
                    <div>
                      <p className={`font-semibold text-sm ${active ? `text-${cat.color}-300` : "text-white"}`}>{cat.label}</p>
                    </div>
                  </div>
                      ...
                      {cat.links.map(link => {
                        ...
                        return (
                          <Link key={link.href + link.label} to={link.href}
                            className="flex items-center gap-2 text-xs text-slate-300 hover:text-white bg-white/5 hover:bg-white/10 rounded-lg px-3 py-2 transition-all"
                          >
                            <LIcon className="w-3.5 h-3.5 text-slate-400" />
                            {link.label}
          ...
          {reporteCategoria === "financiero" && (
            <div className="bg-arandu-dark-light border border-emerald-500/20 rounded-xl p-5">
              <h3 className="text-emerald-400 font-semibold text-sm mb-4 flex items-center gap-2">
                <BarChart3 className="w-4 h-4" /> Reportes Financieros
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {[
                  { title: "Balance General", desc: "Ingresos vs egresos, IVA fiscal, resumen mensual y anual", href: "/admin/balance", color: "text-emerald-400", icon: TrendingUp },
                  { title: "Facturas Emitidas", desc: "Historial de facturas de ventas y su estado de cobro", href: "/admin/ventas", color: "text-cyan-400", icon: Receipt },
                  { title: "Presupuestos", desc: "Seguimiento de presupuestos enviados y aprobados", href: "/admin/ventas", color: "text-blue-400", icon: ClipboardList },
                  { title: "Compras y Egresos", desc: "Registro de compras, costos fijos y sueldos", href: "/admin/egresos", color: "text-orange-400", icon: TrendingDown },
                ].map(r => {
                  const RIcon = r.icon;
                  return (
                    ...
                    >
                      ...
                      <ExternalLink className="w-4 h-4 text-slate-600 group-hover:text-emerald-400 transition-colors mt-0.5 flex-shrink-0" />
                    </Link>
                  );
                })}
              </div>
          ...
          {reporteCategoria === "inventario" && (
            <div className="bg-arandu-dark-light border border-blue-500/20 rounded-xl p-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {[
                  { title: "Productos y Stock", desc: "Estado actual del inventario, niveles de stock y valorización", href: "/admin/inventario", icon: Package, color: "text-blue-400" },
                  { title: "Compras con stock", desc: "Compras que afectaron el inventario de productos", href: "/admin/egresos", icon: DollarSign, color: "text-orange-400" },
                ].map(r => {
                  const RIcon = r.icon;
                  return (
                    <Link key={r.title} to={r.href}
                    ...
                    >
                      ...
                      <ExternalLink className="w-4 h-4 text-slate-600 group-hover:text-blue-400 transition-colors mt-0.5 flex-shrink-0" />
                    </Link>
                  );
                })}
              </div>
          ...
          <div className="bg-arandu-dark-light border border-white/5 rounded-xl p-5">
            <h3 className="text-white font-heading font-semibold mb-4">Seleccionar Empresas</h3>
              ...
              <div className="flex gap-2 ml-auto flex-wrap">
                ...
                {isAdmin && (
                  <Button onClick={() => fetchReportData(true)} className="bg-amber-600 hover:bg-amber-700 text-white" data-testid="preview-with-cred">
                    <Lock className="w-4 h-4 mr-2" /> Vista con Credenciales
                  </Button>
                )}
              </div>
            </div>
      ...
      {tab === "alertas" && (
        <div className="space-y-4">
          <div className="flex flex-wrap gap-3 mb-4">
            <select value={filterEmpresa} onChange={(e) => setFilterEmpresa(e.target.value)} className="bg-arandu-dark border border-white/10 text-white rounded-lg px-3 py-2 text-sm">
              <option value="">Todas las empresas</option>
              {empresas.map(e => <option key={e.id} value={e.id}>{e.nombre}</option>)}
            </select>
            <Button onClick={openNewAlert} className="bg-red-600 hover:bg-red-700 text-white ml-auto" data-testid="new-alert-btn"><Plus className="w-4 h-4 mr-2" /> Nueva Alerta</Button>
          </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={(e) => e.target === e.currentTarget && setShowAlertForm(false)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-arandu-dark-light border border-white/10 rounded-xl w-full max-w-lg">
              ...
              <div className="p-4 space-y-3">
                <div><label className="text-slate-400 text-xs mb-1 block">Empresa *</label>
                  <select value={alertForm.empresa_id} onChange={(e) => setAlertForm({...alertForm, empresa_id: e.target.value})} className="w-full bg-arandu-dark border border-white/10 text-white rounded-lg px-3 py-2 text-sm" data-testid="alert-empresa"><option value="">Seleccionar...</option>{empresas.map(e => <option key={e.id} value={e.id}>{e.nombre}</option>)}</select></div>
                ...
                <div><label className="text-slate-400 text-xs mb-1 block">Activo (opcional)</label>
                  <select value={alertForm.activo_id} onChange={(e) => setAlertForm({...alertForm, activo_id: e.target.value})} className="w-full bg-arandu-dark border border-white/10 text-white rounded-lg px-3 py-2 text-sm"><option value="">Ninguno</option>{activos.filter(a => !alertForm.empresa_id || a.empresa_id === alertForm.empresa_id).map(a => <option key={a.id} value={a.id}>{a.nombre}</option>)}</select></div>
